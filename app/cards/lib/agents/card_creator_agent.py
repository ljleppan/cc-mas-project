...
from cards.lib.agents.learning_utils import card_as_row, data_as_numpy_array


class CardCreatorAgent:
    """
    Agent that creates individual HearthStone Cards based on an inspiring set.

    The inspiring set is a random subset of the local database. More specifically,
    ...
    def __init__(self, subset_size=100):
        """
        Creates a new :class:`CardCreatorAgent`.

        :param subset_size: Size of inspiring set, sampled from all known real
                            cards at random
        """
    ...
    def _load_card_subset(self, subset_size):
        ...
        If data is not cached in a numpu matrix on disc, the cache will be
        created. In this case, the operation will the considerable time.
    ...
    def _memoize(self, card):
        ...
        The new card will replace one item of the inspiring set at random.

        :param card: The card to memoize
        """
    ...
    def _learn_coeffs(self):
        """
        Learn coefficients for valuation of cards based on the inpiring set.
        """
    ...
    def act(self):
        ...
        :math:`P(x)` here is the Poisson distribution,
        :math:`P\\left( x \\right) = \\frac{{e^{ - \\lambda } \\lambda ^x }}{{x!}}`

        Card health and attack values are drawn from a truncated normal distribution.

        Card rarity is determined by the difference between estimated value and
        mana cost, so that cards worth more than their mana cost are given higher
        rarity ratings.

        Card name is generated by sampling from parts of existing card names.
        Similarly, card art is sampled from existing card art. Card race is random,
        with 50 % chance on the card having no race.
    ...
    def _set_random_image(self, card):
        '''Sets random image from existing images for given card.

        :param dict card: card to set the image to
        '''
        ...
        card['image'] = choice(urls)

    def _set_random_race(self, card):
        '''Sets random race for given card.

        :param dict card: card to set the race to
        '''
        if random() < 0.5:
    ...
    def _generate_name(self, card):
        '''Very simple method to create name for a given card.

        :param dict card: card to set the name to
        '''
        ...
        if random() < 0.66:
            name = choice(first) + " "  + choice(second)
        else:
            name = choice(second)
        if random() < 0.25:
            name += " of the " + choice(second)
        card['name'] = name
    ...
    def _generate_mechanics(self, card, max_mechanics=4):
        '''Selects radom mechanics for a given card from existing mechanics pool. The
        amount of mechanics is sampled from poisson distribution (lambda = 2).

        :param dict card: card to set the mechanics to
        :param int max_mechanics: maximum amount of mechanics allowed
        '''
